...
function medal(n: number) { return n===1?"🥇":n===2?"🥈":n===3?"🥉":n; }
...
const MOCK_LB: Lb = {
  topChatters: [
    { name: "alpha", value: 1243 }, { name: "beta", value: 996 }, { name: "gamma", value: 842 },
    { name: "delta", value: 701 }, { name: "epona", value: 560 }, { name: "mika", value: 420 }
  ],
  topDonors: [
    { name: "superfan", value: 180 }, { name: "natsu", value: 120 }, { name: "sora", value: 95 }
  ],
  topSubs: [
    { name: "neo", value: 36 }, { name: "jin", value: 20 }, { name: "ayan", value: 14 },
  ],
};
const MOCK_ME = {
...
export default function LeaderboardsPage() {
  ...
  const { data, isLoading } = useSWR<Lb>("/api/leaderboards?mock=1", fetcher, { refreshInterval: 20000 });
    ...
    <div className="relative">
      ...
      <div className="mb-5 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-white/70 text-sm">Messages, dons et mois de sub — en temps quasi réel.</p>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <button onClick={()=>setTab("chat")} className={`px-3 py-1.5 rounded-xl border ${tab==="chat" ? "bg-white/15 border-white/20" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>Top chatters</button>
          <button onClick={()=>setTab("tips")} className={`px-3 py-1.5 rounded-xl border ${tab==="tips" ? "bg-white/15 border-white/20" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>Top dons (tips)</button>
          <button onClick={()=>setTab("subs")} className={`px-3 py-1.5 rounded-xl border ${tab==="subs" ? "bg-white/15 border-white/20" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>Top subs (mois)</button>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          {isLoading && !data ? (
            <div className="rounded-2xl bg-white/5 border border-white/10 p-6 text-white/70">Chargement…</div>
          ) : (
            <>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            ...
            <p className="text-sm text-white/70">Connecte-toi pour voir ton rang exact et tes stats personnelles.</p>
          </div>
        </div>
      </div>
    </div>
